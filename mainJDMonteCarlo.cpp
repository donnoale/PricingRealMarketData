...
double poisson(double T, int n, double lambda, double gamma, double delta)	{
...
// this function simulates a compound Poisson process
	...
	lambdat=lambda*(T/static_cast<double>(n));
	unsigned seed = std::chrono::steady_clock::now().time_since_epoch().count();

	mt19937 e(seed);		// A Mersenne Twister pseudo-random generator

	poisson_distribution<int> distrP(lambdat);		// Random number distribution that produces integers according to a Poisson distribution
	int number = distrP(e);
	
	for (int i=1; i<=number; i++)	{
		unsigned seed = std::chrono::steady_clock::now().time_since_epoch().count();

		mt19937_64 e(seed);		// A Mersenne Twister pseudo-random generator

		normal_distribution<double> distrN(gamma, delta);		// Random number distribution that produces floating-point values according to a normal distribution
		double z = distrN(e);
		size = exp(z);
		jump = jump*size;
	}
...
double riskyStockPoint(double S0, double sigma, double T, double r, int n, double lambda, double gamma, double delta)
{

// this function computes the price of a stock after a given time period in a jump-diffusion framework
...
double result = S0 * exp((r - lambda * (exp(gamma + (0.5*delta*delta)) - 1) - (sigma*sigma*0.5)) * T + sigma * pow(T, 0.5) * number)*poisson(T, n, lambda, gamma, delta);
return result;
}
...
// this function runs the Monte Carlo simulation in the jump-diffusion framework
...
cin>>nIter;
cout<<"Type your lambda per year"<<endl;
cin>>lambda;
cout<<"Type your gamma"<<endl;
cin>>gamma;
cout<<"Type your delta"<<endl;
cin>>delta;
cout<<endl;
...
for (int a=1; a<=nIter; a++)
{
	for (int i=1; i<=n; i++)
	{
		S[i] = riskyStockPoint(S[i-1], sigma, dt, r, n, lambda, gamma, delta);
	}
